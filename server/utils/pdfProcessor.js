...
async function uploadPageToCloudinary(imagePath, bookId, pageNumber) {
  try {
    ...
    return {
      pageId,
      uniquePageId: pageId, // Add this for database compatibility
      pageNumber,
...
async function processPdfAndExtractInfo(pdfPath) {
  try {
    ...
    let bookInfo;
    try {
      bookInfo = await extractBookInfo(coverText);
      ...
      // Generate generic book information
      bookInfo = await generateGenericBookInfo(coverText);
...
async function processPdfAndUploadPages(pdfPath, bookId) {
  try {
    ...
    console.log("💾 Keeping local images in local_books folder for user access");
